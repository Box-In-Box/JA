...
public class PhotonNetworkManager : MonoPunCallbacksSingleton<PhotonNetworkManager>, ILobbyCallbacks, IInRoomCallbacks, IOnEventCallback
{
    /// <summary>
    ...
    [field : Title("[ Photon Pool ]", "온라인 생성 프리팹 할당 필수")]
    ...
    public Action<Chat_Message> msgAction { get; set; }
...
#region 채널 입장 실패
    ...
    {
        var channel = PopupManager.instance.Open<ChannelPopup>();
...
#region MyRoom
    ...
    public void GetMyRoomMessage(EventData photonEvent)
    {
        ...
        Hashtable msgHash = photonEvent.CustomData as Hashtable;
        PhotonMyRoomCode code = (PhotonMyRoomCode)msgHash["code"];
    }
