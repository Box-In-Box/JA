...
public class ChatManager : View, IChatReceiver
{
    ...
    [field: Title("[ Message Info(Buffer) ]")]
    ...
    [field: Title("[ Prefabs ]")]
    [field: SerializeField] public GameObject ChatArea_MinePrefab { get; private set; }
    [field: SerializeField] public GameObject ChatArea_OtherPrefab { get; private set; }
    [field: SerializeField] public GameObject DateAreaPrefab { get; private set; }
...
#region Chatting
    ...
    public void Chat(bool isSend, string msg, string user, Texture2D picture)
    {
        ...
        Instantiate(isSend ? ChatArea_MinePrefab : ChatArea_OtherPrefab).TryGetComponent<ChatArea>(out ChatArea area);
        ...
        if (lastArea != null && lastArea.time.Substring(0, 10) != area.time.Substring(0, 10)) {
            Transform curDateArea = Instantiate(DateAreaPrefab).transform;
