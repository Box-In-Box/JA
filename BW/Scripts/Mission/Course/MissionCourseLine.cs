...
public class MissionCourseLine : MonoBehaviour
{
    ...
    [field: Title("[ Prefabs ]")]
    [field: SerializeField] public GameObject CourseLineRendererPrefab { get; private set; }
    ...
    public void CourseLineRenderer(int missionId)
    {
        ...
        if (routesType == "finished" || progressData.missionProgress < 0) return;
    ...
    {
        Instantiate(CourseLineRendererPrefab).TryGetComponent<LineRenderer>(out LineRenderer lineRenderer);
